...
export class InfiniteScrollDirective<T> implements OnInit, DoCheck, OnDestroy {
  private _ngFor =
      new NgForOf<T>(this._viewContainer, this._template, this._differs);
  ...
  constructor(private _viewContainer: ViewContainerRef,
              ...
              private _differs: IterableDiffers, private zone: NgZone) {
  ...
  set infiniteScrollEnd(obs$: Observable<NgIterable<T>>) {
    ...
    this._subscriptionEnd =
        this._end$.pipe(exhaustMap(() => obs$))
            .subscribe(this.newItems, () => this.newItems([]));
  }

  get ngForTrackBy(): TrackByFunction<T> { return this._ngFor.ngForTrackBy; }

  ngOnInit() {
    this._subscriptionLoading =
        this._loading$
  ...
  ngDoCheck() { this._ngFor.ngDoCheck(); }

  ngOnDestroy() { this.destroySubscriptions(); }
  ...
  private update(outside: boolean) {
    if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight &&
      ...
      if (this.position < this._items.length) {
        if (outside) {
          this.zone.run(() => { this.updateItems(); });
        } else {
          this.updateItems();
        }
